...
namespace PadForge.ViewModels
{
    ...
    public partial class DevicesViewModel : ViewModelBase
    {
        ...
        // ─── v3.3 gyro tuning UI bindings ───
        // Per-device sliders for H/V sensitivity and deadzone. Mirror
        // the UserDevice fields; pushed back to UserDevice in the
        // setter so changes take effect immediately on the polling
        // thread without waiting for autosave.
        ...
        public double GyroSensitivityH
        {
            ...
            set
            {
                if (SetProperty(ref _gyroSensitivityH, value))
                {
                    PushGyroTuningToDevice();
        ...
        public double GyroSensitivityV
        {
            ...
            set
            {
                if (SetProperty(ref _gyroSensitivityV, value))
                {
                    PushGyroTuningToDevice();
        ...
        /// <summary>Derived: degrees of physical rotation per one full
        /// screen turn at the current H sensitivity. Steam-style readout
        /// next to the multiplier slider — gives the user a real-world
        /// reference instead of an abstract "× value." Calibrated to
        /// 360° baseline at multiplier 1.0 (matches the rate-integrated
        /// virtual-stick mapping where 1× equates one physical turn to
        /// one stick-deflection cycle).</summary>
        ...
        public double GyroDeadZoneDegPerSec
        {
            ...
            set { if (SetProperty(ref _gyroDeadZoneDegPerSec, value)) PushGyroTuningToDevice(); }
        }
        ...
        public double GyroSmoothingAlpha
        {
            ...
            set { if (SetProperty(ref _gyroSmoothingAlpha, value)) PushGyroTuningToDevice(); }
        }
        ...
        public double GyroAcceleration
        {
            ...
            set { if (SetProperty(ref _gyroAcceleration, value)) PushGyroTuningToDevice(); }
        }
        ...
        public string GyroOutputCurve
        {
            ...
            set { if (SetProperty(ref _gyroOutputCurve, value ?? "Linear")) PushGyroTuningToDevice(); }
        }

        /// <summary>The five Steam-style output curves users pick between.</summary>
        ...
        // Live calibrated rate (deg/s) for the selected device's three
        // gyro axes — refreshed by InputService on the UI tick so the
        // user can dial sensitivity sliders visually.
        ...
        /// <summary>Action InputService wires up so slider edits land on
        /// the live <see cref="PadForge.Engine.Data.UserDevice"/> + flag
        /// settings dirty for autosave. Stays null when no Devices page
        /// is mounted; setters fire safely in that case.</summary>
        public Action GyroTuningPushedToDevice { get; set; }

        private bool _suppressGyroTuningPush;

        /// <summary>Load tuning values from a UserDevice into the VM
        /// without triggering the push-back callback. Used when the
        /// selected device changes.</summary>
        public void LoadGyroTuningFromDevice(PadForge.Engine.Data.UserDevice ud)
        {
            _suppressGyroTuningPush = true;
            try
            {
                GyroSensitivityH = ud?.GyroSensitivityH > 0 ? ud.GyroSensitivityH : 1.0;
                GyroSensitivityV = ud?.GyroSensitivityV > 0 ? ud.GyroSensitivityV : 1.0;
                GyroDeadZoneDegPerSec = ud?.GyroDeadZoneDegPerSec >= 0
                    ? ud.GyroDeadZoneDegPerSec : 3.0;
                GyroSmoothingAlpha = ud?.GyroSmoothingAlpha >= 0 ? ud.GyroSmoothingAlpha : 0.0;
                GyroAcceleration = ud?.GyroAcceleration >= 0 ? ud.GyroAcceleration : 0.0;
                GyroOutputCurve = string.IsNullOrEmpty(ud?.GyroOutputCurve)
                    ? "Linear" : ud.GyroOutputCurve;
            }
            finally { _suppressGyroTuningPush = false; }
        }

        private void PushGyroTuningToDevice()
        {
            if (_suppressGyroTuningPush) return;
            GyroTuningPushedToDevice?.Invoke();
        }

...
namespace SDL3
{
    ...
    public static class SDL
    {
        ...
        // ─────────────────────────────────────────────
        //  Bulk state snapshot (fork SDL#16, since 3.6.0)
        // ─────────────────────────────────────────────

        /// <summary>One touchpad-0 finger slot inside the bulk snapshot.
        /// Blittable mirror of SDL_GamepadBulkFinger (16 bytes): the slot
        /// index is the finger identity.</summary>
        [StructLayout(LayoutKind.Sequential)]
        public struct SDL_GamepadBulkFinger
        {
            public byte down;       // C bool
            public byte padding1, padding2, padding3;
            public float x, y, pressure;
        }

        /// <summary>Blittable mirror of the fork's SDL_GamepadBulkState.
        /// 168 bytes, alignment 8, offsets compile-time asserted on the C
        /// side; the size is re-checked here before the first call so a
        /// layout drift disables the path instead of corrupting the
        /// stack.</summary>
        [StructLayout(LayoutKind.Sequential)]
        public unsafe struct SDL_GamepadBulkState
        {
            public ulong timestamp;
            public ulong gyro_timestamp;
            public ulong accel_timestamp;
            public ulong accel_l_timestamp;
            public uint buttons;      // bit N = SDL_GamepadButton N held
            public uint raw_buttons;  // bit N = raw joystick button N held
            public uint capsense;     // bit N = capsense channel N touched
            public byte connected;    // C bool
            public byte raw_hat;
            public byte num_raw_axes;
            public byte num_fingers;
            public fixed short axes[6];
            public fixed short raw_axes[16];
            public byte has_gyro, gyro_enabled, has_accel, accel_enabled,
                        has_accel_l, accel_l_enabled, padding1, padding2;
            public fixed float gyro[3];
            public fixed float accel[3];
            public fixed float accel_l[3];
            public SDL_GamepadBulkFinger finger0;
            public SDL_GamepadBulkFinger finger1;
        }

        [DllImport(lib, CallingConvention = CallingConvention.Cdecl, EntryPoint = "SDL_GetGamepadBulkState")]
        [return: MarshalAs(UnmanagedType.U1)]
        private static extern bool _SDL_GetGamepadBulkState(IntPtr gamepad, out SDL_GamepadBulkState state);

        // 0 = unprobed, 1 = present, -1 = absent (stock SDL) or layout
        // mismatch. Probe result is process-wide. PADFORGE_NO_BULK=1
        // disables the path at launch (regression bisect switch).
        private static int s_bulkProbe =
            Environment.GetEnvironmentVariable("PADFORGE_NO_BULK") == "1" ? -1 : 0;

        /// <summary>Fills a coherent full-state snapshot in ONE native
        /// crossing when the fork export exists. Returns false (and the
        /// caller uses the classic getter sequence) on stock SDL, on a
        /// managed/native layout mismatch, or when the call itself fails.
        /// Values are byte-identical to the getter sequence by construction
        /// (the fork calls the same getters under one lock hold).</summary>
        public static bool SDL_TryGetGamepadBulkState(IntPtr gamepad, out SDL_GamepadBulkState state)
        {
            if (s_bulkProbe < 0) { state = default; return false; }
            if (s_bulkProbe == 0)
            {
                unsafe
                {
                    if (sizeof(SDL_GamepadBulkState) != 168)
                    {
                        s_bulkProbe = -1;
                        state = default;
                        return false;
            ...
            try
            {
                bool ok = _SDL_GetGamepadBulkState(gamepad, out state);
                s_bulkProbe = 1;
                return ok;
            }
        ...
        public static bool SDL_TryGetGamepadNfcTagUid(IntPtr gamepad, out string uid)
        {
            ...
            try
            {
                ...
                // junk: a real UID is 1..20 lowercase-hex chars,

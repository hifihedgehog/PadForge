...
(function () {
    ...
    function setupTouchpadZone(ov, layout) {
        var zone = document.createElement("div");
        zone.className = "touch-zone";
        ...
        zone.addEventListener("touchstart", function(e) {
            ...
            for (var i = 0; i < e.changedTouches.length; i++) {
                ...
                var pos = normalize(t);
                var finger;
                if (finger0Id === null) { finger0Id = t.identifier; finger = 0; }
                else if (finger1Id === null) { finger1Id = t.identifier; finger = 1; }
                else continue;
                sendMessage({ type: "touchpad", finger: finger, x: pos.x, y: pos.y, down: true });
            }
        });
        ...
        zone.addEventListener("touchend", function(e) {
            ...
            for (var i = 0; i < e.changedTouches.length; i++) {
                var t = e.changedTouches[i];
                var finger;
                if (t.identifier === finger0Id) { finger = 0; finger0Id = null; }
                else if (t.identifier === finger1Id) { finger = 1; finger1Id = null; }
                else continue;
                sendMessage({ type: "touchpad", finger: finger, x: 0, y: 0, down: false });
            }
        });
        ...
        zone.addEventListener("touchcancel", function(e) {
            ...
            for (var i = 0; i < e.changedTouches.length; i++) {
                ...
                if (t.identifier === finger0Id) {
                    finger0Id = null;
                    sendMessage({ type: "touchpad", finger: 0, x: 0, y: 0, down: false });
                } else if (t.identifier === finger1Id) {
                    finger1Id = null;
                    sendMessage({ type: "touchpad", finger: 1, x: 0, y: 0, down: false });
                }
            }
        });

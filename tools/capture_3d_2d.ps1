<#
.SYNOPSIS
    Captures pad-controller-3d and pad-controller-2d screenshots.
    Uses AutomationId "TabController" to click the 3D/visualization tab.
...
function SelectEl($el,[int]$d=800,[string]$lbl) {
    if(-not $el){Write-Host "  !! NOT FOUND: $lbl" -ForegroundColor Red;return $false}
    try{$pat=$el.GetCurrentPattern([System.Windows.Automation.SelectionItemPattern]::Pattern);Write-Host "  Select '$lbl'";$pat.Select();Start-Sleep -Milliseconds $d;return $true}catch{}
...
# Click Controller tab using AutomationId
Write-Host "=== 3D Controller Tab ==="
$ctrlTab = Find1 -A "TabController"
Write-Host "  TabController found: $(if($ctrlTab){'YES'}else{'NO'})"
if ($ctrlTab) {
    $r = $ctrlTab.Current.BoundingRectangle
    Write-Host "  Bounds: X=$([int]$r.X) Y=$([int]$r.Y) W=$([int]$r.Width) H=$([int]$r.Height)"
    ClickEl $ctrlTab -d 2000 -lbl "TabController"
}
Start-Sleep -Milliseconds 2000
Cap "pad-controller-3d"

# 2D toggle
Write-Host "=== 2D Toggle ==="
$uiaWin = $uiaRoot.FindFirst($TC, $pidCond)
$padPage = Find1 -A "PadPageView"
if ($padPage) {
    $ppRect = $padPage.Current.BoundingRectangle
    Write-Host "  PadPageView: X=$([int]$ppRect.X) Y=$([int]$ppRect.Y) W=$([int]$ppRect.Width) H=$([int]$ppRect.Height)"
    $toggleX = [int]($ppRect.X + 52)
    $toggleY = [int]($ppRect.Y + 124)
    Write-Host "  Toggle at ($toggleX,$toggleY)"
